...
function Level(plan) {
  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Use if and else to handle the three cases
       var Actor = actorChars [ch]; //added
       if (Actor)
          this.actors.push(new Actor(new Vector(x,y), ch)); //added
      else if (ch == "x")
        fieldType = "wall";
  ...
  ths.player = this.actors.filter(function(actor) { // )????
...
Level.prototype.playerTouched = function (type, actor){ //added
  if (type == "coin"){
    this.actors = this.actor.filter(function(other){
      return other !=actor;
  }); // add ); if script not work
}
};
